			...
			<div class="col-sm-8">
				<div class="text-right">
					<div class="infont form-group icons-box navbar-right">
						<button class="btn btn-success" id="btnRegist">SR등록</button>
					</div>
				</div>
			</div>
		</div>
					...
					<form id="F1" class="form-horizontal">
						...
						<div class="row">
							<div class="col-md-12">
								...
								<label class='col-sm-2' style="padding-top:7px;">작업대상</label>
								<div id="PATH" class='col-sm-10' style="padding-top:7px;"></div>
							</div>
						</div>
						<div class='hr-line-dashed'></div>
						<div class="row">
		                	<div class="col-md-12">
		                		<div>
									...
									<div class='col-sm-10'>
										<select id="SR_APPLY_ENV" class="form-control required">
											${CDE00021}
										</select>
									</div>
								</div>
						...
						<div class="row">
		                	<div class="col-md-12">
		                		<div>
									<label class='col-sm-2' style="padding-top:7px;">시스템명<i class="text-danger"></i></label>
									<div class='col-sm-10'>
										<input id='SR_SYSTEM_NAME' type='text' class='form-control'>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
		                	<div class="col-md-12">
		                		<div>
									<label class='col-sm-2' style="padding-top:7px;">작업근거<i class="text-danger"></i></label>
									<div class='col-sm-10'>
										<input id='SR_WORK_REASON' type='text' class='form-control'>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-md-6">
			                	<div>
									<label class='col-sm-4' style="padding-top:7px;">완료예정일</label>
									<div class='col-sm-8'>
										<div class="date input-group" id="datepicker">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
											<input type="text" class="form-control" id="SR_COMPLETE_DATE" data-inputmask="'alias': 'yyyy-mm-dd'" disabled>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div>
									<label class='col-sm-2' style="padding-top:7px;">작업상태<i class="text-danger">*</i></label>
									<div class='col-sm-10'>
						...
						<div class="row">
							<div class="col-md-12">
			                	<div>
									...
									<div class='col-sm-10'>
										<input id="SR_AST_NAME" type="text" class="form-control required reqdis" disabled>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="col-md-6">
			                	<div>
									...
									<div class='col-sm-8'>
										<div class="date input-group" id="datepicker">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
											<input type="text" class="form-control resdis" id="SR_COMPLETE_DATE" data-inputmask="'alias': 'yyyy-mm-dd'" value="${today}">
										</div>
									</div>
								</div>
							</div>
						...
						<div class="row">
							...
							<div class="col-md-6">
								<div>
									<label class='col-sm-4' style="padding-top:7px;">담당자<i class="text-danger"></i></label>
									<div class='col-sm-8'>
										<select id="SR_MANAGER_ID" class="form-control reqdis">
											${ITOS}
										</select>
									</div>
								</div>
...
<script>
	...
	$.fncDrawHrTree = function() {
		...
		$('#humanResourceTree').jstree({
			'core' : {
				...
				'data' : {
	            	 'url' : function(node) {
	                     return '${pageContext.request.contextPath}/data/asset/assetcustom.rts';
					  },
	                 'data' : function(node) {
	...
	$.fncGrid1 = function() {
		...
		$.ritusGrid({
			...
			url: '${pageContext.request.contextPath}/data/work/srrequestlistbyasset.rts',
			postData: {
				category_id : 'RSR00001',
				ast_id : $.g_selectedNode,
				startDate : startDate,
				endDate : endDate,
		    ...
		    onCellSelect: function(rowid, icol, cellcontent, e) {
				...
				} else if ( man_id == $.g_usrID ) {	// $.g_usrID -> 로그인된 아이디
		    	...
		    	if ( complete_date == '' ) {
		    		...
		    		$('#detailModal2 .btnMod').show();
		    		if ( $.g_usrID != man_id && $.g_usrID != req_id ) {
		    			$('#detailModal2 .btnMod').hide();
		    			$('#detailModal2 .btnUploadFiles').prop('disabled', true);
		    		}
	...
	$.fncGetData = function(obj, data_id) {
		...
		$.ritusAjax({
			...
			success: function(data) {
				$(obj + ' .form-control').each(function() {
					// $(obj + ' .form-control') 의 id값을 가져옴
					var id = $(this).attr('id');
					
					// data.가져온 id값을 d에 집어넣음
					var d = eval('data.' + id);
					
					// TextArea의 엔턴 눌렀을때 <br>키에 대한 replaceAll
	...
	$.fncSaveData = function(obj) {
		...
		if ( url == '${pageContext.request.contextPath}/data/common/create.rts' ) {
			var SR_AST_NAME = sendData.SR_AST_NAME;
			var requestDate = sendData.SR_REQ_DATE;
			$.ritusAjax({
				url: '${pageContext.request.contextPath}/data/work/srnumber.rts',
				data: { SR_AST_NAME : SR_AST_NAME, SR_REQ_DATE : requestDate },
				dataType: 'json',
				success: function(data) {
					sendData.SR_NUMBER = data.SR_NUMBER;
					$.ritusAjax({
						url: url,
						data: sendData,
						dataType: 'json',
...
$(function() {
	...
	$('#btnRegist').click(function() {
		if ( $.g_selectedPath == '' || $.g_selectedNode == 'CDE00020008') {
			alert('작업대상 자산을 선택해주세요.');
			return;
		}
		
		$.fncClearData('#detailModal');
		
		$('#detailModal #file_text').text('');
		
		$('#detailModal #SR_AST_ID').val($.g_selectedNode);
		$('#detailModal #SR_AST_NAME').val($.g_selectedNodeText);
		$('#detailModal #PATH').html($.g_selectedNodeText);
		$('#detailModal').modal({backdrop: 'static', keyboard: false});
	});
	
	$('#detailModal .btnAdd').click(function() {
	...
	$('#detailModal .btnUploadFiles').click(function() {
		$.g_objModal = '#detailModal';
		$('#uploadMultipleFile').click();
	});
	
	$('#detailModal2 .btnUploadFiles').click(function() {
		$.g_objModal = '#detailModal2';
		$('#uploadMultipleFile').click();
	});
